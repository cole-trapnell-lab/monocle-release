...
<<lung_beam_branched_heatmap, include=TRUE, eval=TRUE, fig.width = 4, fig.height = 8, fig.align="center", warning=FALSE>>=
plot_genes_branched_heatmap(lung[row.names(subset(BEAM_res, qval < 0.05)),], branch_point = 1, num_clusters = 4, 
      cores = detectCores(), use_gene_short_name = T, show_rownames = T)
@ %def 

<<lung_beam_branched_pseudotime, include=TRUE, eval=TRUE, fig.width = 6, fig.height = 3, fig.align="center", warning=FALSE>>=
plot_genes_branched_pseudotime(lung[1:5,], branch_point=1, ncol=2)
@ %def 
