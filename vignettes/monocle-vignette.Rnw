...
<<identify_cell_type, fig.width = 3, fig.height = 2, fig.align="center", eval=TRUE>>=
...
Now that we have tagged the cells, it's easy to quantify the level of fibroblast ``contaminatation'' in the HSMM culture:
...
Monocle also gives you some tools to cluster cells by type even if you don't 
have a good set of marker genes to distinguish them. The \Rfunction{clusterCells} groups cells by similarity in expression profiles. Cells of the same type should group together. Sometimes, however, it may be necessary to subtract the differences in expression between cells that arise due to other sources of variation in the experiment. In the muscle experment, the cells were collected on different days and cells at the beginning of the experiment were placed in different culture media. Monocle allows you to subtract these effects by specifying a model formula. 
<<cluster_cells, fig.width = 3, fig.height = 4, fig.align="center", eval=TRUE>>=

MYF5_id <- row.names(subset(fData(HSMM), gene_short_name == "MYF5"))
MEF2C_id <- row.names(subset(fData(HSMM), gene_short_name == "MEF2C"))
...
HSMM <- clusterCells(HSMM,
                     num_clusters=50, 
                     num_reduced_dims=3,
                     frequency_thresh=0.25,
                     cell_type_hierarchy=cth,
                     clustering_genes=row.names(subset(marker_diff, qval < 1e-3)),
                     residualModelFormulaStr="~Media")
...
clustering_genes <- row.names(subset(disp_table, dispersion_empirical >= 2 * dispersion_fit))
HSMM <- setOrderingFilter(HSMM, clustering_genes)
