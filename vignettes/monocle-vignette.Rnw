...
HSMM <- reduceDimension(HSMM, max_components=2, num_dim = 7, 
...
HSMM@auxClusteringData[["tSNE"]]$variance_explained <- NULL
plot_pc_variance_explained(HSMM, return_all = F) # norm_method = 'log',
HSMM <- reduceDimension(HSMM, max_components=2, num_dim = 3, reduction_method = 'tSNE', 
                        residualModelFormulaStr="~Media + num_genes_expressed", verbose = T) 
HSMM <- clusterCells(HSMM,
  num_clusters=2,
...
HSMM@auxClusteringData[["tSNE"]]$variance_explained <- NULL
plot_pc_variance_explained(HSMM, return_all = F) # norm_method = 'log',
HSMM <- reduceDimension(HSMM, max_components=2, num_dim = 7, reduction_method = 'tSNE', residualModelFormulaStr="~Media + num_genes_expressed", verbose = T) #norm_method = 'log'
...
During our reanalysis of many sc RNA-seq data, we find that informative genes for biological
processes often form a block structure among clusters of cells involved in the process.
...
First, we need to set the superset of feature genes as genes expressed in at least $5\%$ of all the cells. 
...
<<perform tSNE dimension reduction, fig.width = 3, fig.height = 3, fig.align="center", eval=TRUE, warning=FALSE>>=
  HSMM_myo <- reduceDimension(HSMM_myo, max_components=2, norm_method = 'log', num_dim = 8, reduction_method = 'tSNE', verbose = T)
@ %def
...
HSMM_myo <- clusterCells(HSMM_myo,  rho_threshold = 2, delta_threshold = 4, skip_rho_sigma = T, verbose = F)
@ %def
...
clustering_DEG_genes <- differentialGeneTest(HSMM_myo, fullModelFormulaStr = '~Cluster', cores = detectCores() - 2)
...
HSMM_ordering_genes <- row.names(clustering_DEG_genes_subset)[order(clustering_DEG_genes_subset$qval)][1:1000] #1971
...
One observation we can immediately make is that reverse embedded data are smoother and the drop out in the original data are largely alleviated. Whether or not we can apply the de-noised data for downstream analysis or for data imputation for dropout could be future directions worth exploring. 
