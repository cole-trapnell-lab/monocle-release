...
HSMM <- reduceDimension(HSMM, max_components=2, num_dim = 6, reduction_method = 'tSNE', 
...
Using the top 1000 genes for ordering produces a trajectory that's highly similar to the one we obtained with unsupervised methods, but it's a little ``cleaner'': the small branches are gone.
<<semi_sup_ordering_trajectory, eval=TRUE, fig.width = 6, fig.height = 4, fig.align="center">>=
...
HSMM_filtered <- HSMM_myo[expressed_genes, 
                          pData(HSMM_myo)$State %in% c(1,2,4)]
