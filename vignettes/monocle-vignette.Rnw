...
<<build_cell_data_Set_RPC, cache=FALSE, eval=TRUE>>=
pd <- new("AnnotatedDataFrame", data = HSMM_sample_sheet)
fd <- new("AnnotatedDataFrame", data = HSMM_gene_annotation)

rpc_matrix <- relative2abs(as.matrix(HSMM_expr_matrix))
...
<<estimate_size_and_dispersion, cache=TRUE, eval=TRUE >>=
...
HSMM <- HSMM[,row.names(subset(pData(HSMM), Total_mRNAs >= 10000 & Total_mRNAs <= 25000))]
HSMM <- detectGenes(HSMM, min_expr = 0.1)
@ %def
