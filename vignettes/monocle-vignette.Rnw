...
We can plot a couple of these genes, such as \emph{Pdpn} and \emph{Sftpb} using the \Rfunction{plot\_genes\_branched\_pseudotime} function, which works a lot like the \Rfunction{plot\_genes\_in\_pseudotime} function, except it shows two kinetic trends, one for each lineage, instead of one. Note that \emph{Cdk1}, a cell cycle genes is 

<<lung_beam_branched_pseudotime, include=TRUE, eval=TRUE, fig.width = 4, fig.height = 4, fig.align="center", warning=FALSE>>=
lung_genes <- row.names(subset(fData(lung), gene_short_name %in% c("Cdk1", "Sftpb", "Pdpn")))
