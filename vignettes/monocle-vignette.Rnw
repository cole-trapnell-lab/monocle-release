...
plot_genes_branched_heatmap(lung[row.names(subset(BEAM_res, qval < 0.1)),], num_clusters = 4, 
      cores = detectCores(), use_gene_short_name = T, show_rownames = T)

#the symbol of the bifurcation_time represent when lineage has high expression
bifurcation_time <- abs(BEAM_res$Bifurcation_time_point)
names(bifurcation_time) <- BEAM_res$gene_short_name #set name of the vector to that of the bifurcation point

plot_genes_branched_pseudotime(lung[1:4, ], add_pval = T, ncol = 2,
...
HSMM <- estimateDispersions(HSMM, cores=6)
@ %
...
diff_test_res <- differentialGeneTest(HSMM_myo[marker_genes,], 
                                      fullModelFormulaStr="~Media", cores=2)
