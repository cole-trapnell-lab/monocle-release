...
<<init_treutlein, include=TRUE, eval=TRUE>>=
...
<<collect_metrics_lung, include=TRUE, eval=TRUE>>=
...
<<order_lung, include=TRUE, eval=TRUE, fig.width = 4, fig.height = 4, fig.align="center", warning=FALSE>>=
...
<<lung_beam_test, include=TRUE, eval=TRUE, fig.width = 4, fig.height = 4, fig.align="center", warning=FALSE>>=
BEAM_res <- BEAM(lung, cores=8)
head(BEAM_res)
plot_genes_branched_heatmap(lung[row.names(subset(BEAM_res, qval < 0.05)),])
@ %def 
...
<<order_cells, eval=TRUE, warning=FALSE>>=
