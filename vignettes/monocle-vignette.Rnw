...
#prepare your cds and set the clustering genes as genes expressed larger than 5 (you may need to adjust this based on your data)
 <<select_expressed_genes, fig.width = 3, fig.height = 4, fig.align="center", eval=TRUE>>=
 fData(HSMM)$use_for_ordering <- fData(HSMM)$num_cells_expressed > 0.05 * ncol(HSMM)
@ %def
 
#procedure to run density peak clustering 
#1. determine how many pca dimension you want: 
<<select_expressed_genes, fig.width = 3, fig.height = 4, fig.align="center", eval=TRUE>>=
plot_pc_variance_explained(HSMM, return_all = F) #look at the plot and decide how many dimensions you need. It is determined by a huge drop of variance at that dimension. pass that number to num_dim in the next function. I used 10 there. 
@ %def

#2. run reduceDimension with tSNE as the reduction_method 
<<select_expressed_genes, fig.width = 3, fig.height = 4, fig.align="center", eval=TRUE>>=
HSMM <- reduceDimension(HSMM, max_components=2, norm_method = 'log', num_dim = 9, reduction_method = 'tSNE', verbose = T)
@ %def

#3. initial run of clusterCells_Density_Peak
<<select_expressed_genes, fig.width = 3, fig.height = 4, fig.align="center", eval=TRUE>>=
HSMM <- clusterCells_Density_Peak(HSMM, verbose = T)
@ %def

#4. check the clusters 
<<select_expressed_genes, fig.width = 3, fig.height = 4, fig.align="center", eval=TRUE>>=
plot_cell_clusters(HSMM, color_by = 'as.factor(Cluster)', show_density = F)
plot_cell_clusters(HSMM, color_by = 'as.factor(Hours)', show_density = F)
@ %def

#5. also check the decision plot 
<<select_expressed_genes, fig.width = 3, fig.height = 4, fig.align="center", eval=TRUE>>=
plot_rho_delta(HSMM, rho_threshold = 2, delta_threshold = 10 )
@ %def

#6. re-run cluster and skipping calculating the rho_sigma 
<<select_expressed_genes, fig.width = 3, fig.height = 4, fig.align="center", eval=TRUE>>=
HSMM <- clusterCells_Density_Peak(HSMM, verbose = T,  rho_threshold = 2, delta_threshold = 10, skip_rho_sigma = T)
@ %def
