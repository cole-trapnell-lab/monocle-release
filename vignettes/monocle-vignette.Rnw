...
<<semi_sup_ordering_trajectory, include=TRUE, eval=TRUE, fig.width = 4, fig.height = 2, warning=FALSE, fig.align="center">>=
...
about tools for dealing with branched trajectories in section \cite{BEAM_trajectory}.
...
analysis has been posted on biorxiv (\cite{Qiu2017-nx}).
