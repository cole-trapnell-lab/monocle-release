...
<<lung_beam_kinetics, include=TRUE, eval=TRUE, fig.width = 5, fig.height = 3, fig.align="center", warning=FALSE>>=
qval_sort <- sort(BEAM_res$qval, index.return = T)
plot_genes_branched_pseudotime(lung[qval_sort$ix[1:4], ], add_pval = T, ncol = 2,
                               trajectory_color_by = 'Lineage', 
                               reducedModelFormulaStr = "~ sm.ns(Pseudotime, df=3)",
                               lineage_labels = c("AT1", "AT2"), 
                               bifurcation_time = BEAM_res$Bifurcation_time_point[qval_sort$ix[1:4]]
                                )
@ %def 
