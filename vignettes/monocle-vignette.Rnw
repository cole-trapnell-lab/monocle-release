...
HSMM <- reduceDimension(HSMM, max_components=2, num_dim = 15, reduction_method = 'tSNE', 
...
clustering_DEG_genes_subset <- clustering_DEG_genes[fData(HSMM_myo)$num_cells_expressed > 0.05 
                                                    * ncol(HSMM_myo), ]
@ %def
...
HSMM_ordering_genes <- row.names(clustering_DEG_genes_subset)
[order(clustering_DEG_genes_subset$qval)][1:1000] 
...
that successfully fuse to form myotubes (State 2), and the other to those that fail to 
fully differentiate (State 3). We'll exclude the latter for now, but you can learn more 
...
Monocle 2 uses a technique called \emph{reversed graph embedding}\cite{QiRGE, qiu2017RGE} to learn the 
