...
\textbf{NOTE:} when creating a \Robject{CellDataSet}, the matrix passed in to the featureData variable should have a column verbatim named 'gene\_short\_name'. Otherwise, some functions that accept
your \Robject{CellDataSet} as a parameter will not work.
