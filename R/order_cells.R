...
#' Find the best bindwidth for performing the dimension reduction with diffusion map (algorithm stolen from the German group)
diffusion_maps_bw <- function(cds, pseudo_cnt = 1, neighbours = 1, bw_ini = 0, iter = 100, step = .02, log2 = F){
  if(!is.null(fData(cds)$use_for_ordering))
    cds <- cds[which(fData(cds)$use_for_ordering), ]
    
  if(log2)
    data <- t(log2(exprs(cds) + pseudo_cnt))
  else 
    data <- t(exprs(cds))
...
#' Function to perform the dimension reduction with the diffusion maps 
diffusion_maps <- function(cds, bw = 10, pseudo_cnt = 1, neighbours = .2, log2 = F) {
  if(!is.null(fData(cds)$use_for_ordering))
    cds <- cds[which(fData(cds)$use_for_ordering), ]
  
  if(log2)
    data <- t(log2(exprs(cds) + 1))
  else 
    data <- t(exprs(cds))
...
#' Function to perform the dimension reduction with the diffusion maps 
DM_reduceDimension <- function (cds, num_dims = 2, bw = 10, pseudo_cnt = 1, neighbours = .2, log2 = F, ...)
{
  dm <- diffusion_maps(cds, bw = bw, pseudo_cnt = pseudo_cnt, neighbours = neighbours, log2 = log2, ...)
  
  reducedDimW(cds) <- t(E$vectors)
  reducedDimA(cds) <- t(E$vectors)
  reducedDimS(cds) <- t(E$vectors)
  reducedDimK(cds) <- t(E$vectors)
}

#' Use principal curve to order cells for the datasets have bifurcation
prinOrder_update <- function(cds, progenitor_state = 1, lineage_A = c(1, 2), lineage_B = c(1, 3), method = c(pcurve, principal.curve), ...) {
  S_matrix <- t(reducedDimS(cds))
  
  #check the direction: 
  A_lineage <- S_matrix[pData(cds)$State %in% lineage_A, ]
  B_lineage <- S_matrix[pData(cds)$State %in% lineage_B, ]
  
  A_pc <- method(A_lineage, ...) #, penalty = 2.2, start = 'pca', plot.init = FALSE, fits = TRUE, prnt.fits = TRUE, use.loc = F
  B_pc <- method(B_lineage, ...)
  
  A_lambda <- A_pc$lambda 
  B_lambda <- B_pc$lambda 
  
  #sort the principal curve if not starting from the progenitor cells
  progenitor_cells <- colnames(cds)[pData(cds)$State == progenitor_state]
  lineage_A_cells <- colnames(cds)[pData(cds)$State == setdiff(lineage_A, progenitor_state)]
  lineage_B_cells <- colnames(cds)[pData(cds)$State == setdiff(lineage_B, progenitor_state)]
  
  if(min(A_pc$lambda[row.names(A_lineage) %in% progenitor_cells]) > 0)
    A_lambda[sort(A_pc$lambda, index.return = T)$ix] <- rev(sort(A_pc$lambda))
  if(min(B_pc$lambda[row.names(B_lineage) %in% progenitor_cells]) > 0)
    B_lambda[sort(B_pc$lambda, index.return = T)$ix] <- rev(sort(B_pc$lambda))

  names(A_lambda) <- row.names(A_lineage)
  names(B_lambda) <- row.names(B_lineage)
  A_lambda <- A_lambda[sort(A_lambda, index.return = T)$ix]
  B_lambda <- B_lambda[sort(B_lambda, index.return = T)$ix]
    

  #transform the PC pseudotime: 
  P_0 <- mean(max(A_lambda[progenitor_cells]) + B_lambda[progenitor_cells]) 
  A_lambda[lineage_A_cells] <- A_lambda[lineage_A_cells] - max(A_lambda[progenitor_cells]) + P_0 
  B_lambda[lineage_B_cells] <- B_lambda[lineage_B_cells] - max(B_lambda[progenitor_cells]) + P_0 
  
  A_lambda[progenitor_cells] <- (A_lambda[progenitor_cells] + B_lambda[progenitor_cells]) / 2
  B_lambda[progenitor_cells] <- (A_lambda[progenitor_cells] + B_lambda[progenitor_cells]) / 2
  
  pData(cds)[names(A_lambda), 'pc_Pseudotime'] <- A_lambda
  pData(cds)[names(B_lambda), 'pc_Pseudotime'] <- B_lambda
