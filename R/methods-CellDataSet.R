...
#' data(HSMM)
#' A <- reducedDimA(HSMM)
getSizeFactors <- function(object) {
  sf <- pData(object)$Size_Factor
  names( sf ) <- colnames( exprs(object) )
  sf}
setMethod("sizeFactors", signature(object="CellDataSet"), getSizeFactors)   
...
setReplaceMethod("sizeFactors", signature(object="CellDataSet", value="numeric"), setSizeFactors)   
...
.estimateDispersions <- function(object, modelFormulaStr="expression~1", relative_expr=TRUE, cores=1, dispModelName="blind", ... )
{
...
setMethod("estimateDispersions", signature(object="CellDataSet"), .estimateDispersions)
