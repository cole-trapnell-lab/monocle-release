...
getVarianceStabilizedData <- function( cds, coefs ) {
  ncounts <- exprs(cds)
  ncounts <- t(t(ncounts) / pData(cds)$size_factors)
...
computeVarianceStabilizedValues <- function(cds, model_row_names=NULL, modelFormulaStr="expression~1", cores=1) {
  
  if (cds@expressionFamily@vfamily %in% c("zanegbinomialff","negbinomial", "poissonff", "quasipoissonff"))
  {
    pData(cds)$size_factors <- estimateSizeFactorsForMatrix(round(exprs(cds)))
  }
...
reduceDimension <- function(cds, max_components=2, use_irlba=TRUE, pseudo_expr=1, batch=NULL, covariates=NULL, use_vst=F, ...){
  ...
  if (use_vst == F && cds@expressionFamily@vfamily %in% c("zanegbinomialff","negbinomial", "poissonff", "quasipoissonff"))
  {
