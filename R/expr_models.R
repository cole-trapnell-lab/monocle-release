...
fit_model_helper <- function(x, 
    ...
    tryCatch({
        ...
        if (is.null(backup_expression_family) == FALSE){
          ...
          test_res <- tryCatch({
          if (verbose){
            FM_fit <- VGAM::vglm(as.formula(modelFormulaStr), family=backup_expression_family)
          }else{
            FM_fit <- suppressWarnings(VGAM::vglm(as.formula(modelFormulaStr), family=backup_expression_family))
          }
            FM_fit
          }, 
