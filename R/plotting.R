...
#' Plot complex tree from two or high dimension

#' @export
plot_complex_tree <- function(cds, 
                              color_by="State", 
                              show_tree=TRUE, 
                              show_backbone=TRUE, 
                              backbone_color="black", 
  ...
  if(is.null(lib_info_with_pseudo$Pseudotime)) {
    root_cell <- V(dp_mst)[degree(dp_mst) == 1][1]
  }
  else
    root_cell <- V(dp_mst)[which(lib_info_with_pseudo$Pseudotime == 0)]
  
  if (cds@dim_reduce_type == "ICA"){
    layout_coord <- layout.reingold.tilford(dp_mst, root= root_cell)
  }else if (cds@dim_reduce_type == "DDRTree"){
    layout_coord <- layout.reingold.tilford(dp_mst, root= root_cell)
  }else {
    stop("Error: unrecognized dimensionality reduction method.")
  ...
  if (is.null(layout_coord)){
    stop("You must first call reduceDimension() before using this function")
  }
  
  layout_coord <- as.data.frame(layout_coord)
  colnames(layout_coord) <- c("prin_graph_dim_1", "prin_graph_dim_2")
  
  layout_coord$sample_name <- paste('Y_', row.names(layout_coord), sep = '')
  #ica_space_with_state_df <- merge(layout_coord, lib_info_with_pseudo, by.x="sample_name", by.y="row.names")
  #print(ica_space_with_state_df)
  ...
  edge_df <- merge(layout_coord, edge_list, by.x="sample_name", by.y="source", all=TRUE)
  #edge_df <- layout_coord
  edge_df <- plyr::rename(edge_df, c("prin_graph_dim_1"="source_prin_graph_dim_1", "prin_graph_dim_2"="source_prin_graph_dim_2"))
  edge_df <- merge(edge_df, layout_coord[,c("sample_name", "prin_graph_dim_1", "prin_graph_dim_2")], by.x="target", by.y="sample_name", all=TRUE)
  edge_df <- plyr::rename(edge_df, c("prin_graph_dim_1"="target_prin_graph_dim_1", "prin_graph_dim_2"="target_prin_graph_dim_2"))
  
  data_df <- layout_coord
  row.names(data_df) <- colnames(cds)
  colnames(data_df) <- c("data_dim_1", "data_dim_2")
  data_df$sample_name <- row.names(data_df)
  data_df <- merge(data_df, lib_info_with_pseudo, by.x="sample_name", by.y="row.names")
