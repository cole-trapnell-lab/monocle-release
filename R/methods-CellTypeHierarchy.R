...
classifyCellsHelperCell <- function(cds, cth){
  ...
  cds_pdata <- dplyr::group_by_(dplyr::select_(tibble::rownames_to_column(pData(cds)), "rowname"), "rowname")
...
#' @importFrom dplyr tibble::rownames_to_column select_ do group_by_ inner_join
...
classifyCells <- function(cds, cth, frequency_thresh=NULL, ...) {
  ...
  pData(cds) <- as.data.frame(suppressMessages(inner_join(tibble::rownames_to_column(pData(cds)), class_df)))
...
calculateMarkerSpecificity <- function(cds, cth, remove_ambig=TRUE, remove_unknown=TRUE){
  ...
  cds_pdata <- dplyr::group_by_(dplyr::select_(tibble::rownames_to_column(pData(cds)), "rowname", "CellType"), "CellType")
...
markerDiffTable <- function (cds, cth, residualModelFormulaStr="~1", balanced=FALSE, reclassify_cells=TRUE, remove_ambig=TRUE, remove_unknown=TRUE, verbose=FALSE, cores=1) {
  ...
  if (balanced){
    ...
    for (cell_type in names(cell_type_counts)){
      cell_type_sample <- sample_n(tibble::rownames_to_column(subset(pData(cds), CellType == cell_type)), n_cells)$rowname
