...
void setSeed(unsigned int seed) {
  ...
  rngU = uniform_real_distribution<>(0.0, 1);
}
...
#pragma omp parallel for
  ...
  for(int t=1;t<count;t++){
    ...
    int half = total/2;
    int number = std::fmod(runif(), half) + (total-half);
...
int main(int argc, char**argv){
  ...
  if(parser.cmdOptionExists("-h")){
    Rcpp::Rcout<<"command format:"<<std::endl;
    Rcpp::Rcout<<"landmark_selector -p indptr-file -d data-file -i indices-file -c cores -n landmarks -o output-dir"<<std::endl;
  }

  string indptr_file = parser.getCmdOption("-p");
  if(indptr_file.empty()){
    Rcpp::Rcout<<"missing indptr file"<<std::endl;
    return 0;
  }
  string data_file = parser.getCmdOption("-d");
  if(data_file.empty()){
    Rcpp::Rcout<<"missing data file"<<std::endl;
    return 0;
  }
  string indices_file = parser.getCmdOption("-i");
  if(indices_file.empty()){
    Rcpp::Rcout<<"missing indices file"<<std::endl;
    return 0;
  }
  ...
  if (output_dir.empty()){
    Rcpp::Rcout<<"missing output directory"<<std::endl;
    return 0;
  }

  string line;

  Rcpp::Rcout<<"loading "<<indptr_file<<std::endl;
  ...
  int dim = d_n[0];
  int n = d_n[1];
  Rcpp::Rcout<<"n="<<n<<",dim="<<dim<<std::endl;
  ...
  Rcpp::Rcout<<"loading "<<data_file<<std::endl;
  ...
  Rcpp::Rcout<<"loading "<<indices_file<<std::endl;
